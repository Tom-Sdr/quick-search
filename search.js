...
const dividerLineTemplate = document.createElement("template");
dividerLineTemplate.innerHTML = `
...
function createList(searchQuery) {
  ...
  getTabsByQuery(searchQuery).forEach((tabInfo) => {
    tabListElem.appendChild(createTabDisplayFrom(tabInfo));
  });

  const dividerLine = dividerLineTemplate.content.firstElementChild.cloneNode(true);
  tabListElem.appendChild(dividerLine);
  
  getBookmarksByQuery(searchQuery).forEach((bookmarkInfo) => {
    tabListElem.appendChild(createBookmarkDisplayFrom(bookmarkInfo));
  });
}
...
const tabDisplayTemplate = document.createElement("template");
tabDisplayTemplate.innerHTML = `
...
function createTabDisplayFrom(tabInfo) {
  const tabDisplayInstance = tabDisplayTemplate.content.firstElementChild.cloneNode(true);

  tabDisplayInstance.dataset.tabId = tabInfo.id;
  tabDisplayInstance.querySelector(".tab-icon").src = tabInfo.favIconUrl || "";
  tabDisplayInstance.querySelector(".tab-text").textContent = tabInfo.title;

  return tabDisplayInstance;
}


const bookmarkDisplayTemplate = document.createElement("template");
bookmarkDisplayTemplate.innerHTML = `
  <div class="tab-display">
...
function createBookmarkDisplayFrom(bookmarkInfo) {
  const bookmarkDisplayInstance = bookmarkDisplayTemplate.content.firstElementChild.cloneNode(true);

  bookmarkDisplayInstance.dataset.bookmarkId = bookmarkInfo.id;
  bookmarkDisplayInstance.querySelector(".tab-text").textContent = bookmarkInfo.title;

  return bookmarkDisplayInstance;
}
