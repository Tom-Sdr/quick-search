<script lang="ts">
  ...
  async function getNonPendingTabsOfCurrentWindow(
    searchQuery: string,
  ...
  async function getBookmarks(searchQuery: string) {
    ...
    return (await browser.bookmarks.search({}))
      ...
      .filter((bookmarkNode) => {
        ...
        if (!hasUrl) {
          console.error(
            `bookmark with id ${bookmarkNode.id} does not have url`,
          );
        }
        return hasUrl;
      })
  ...
  async function updateList(
    searchQuery: string,
    updateReason: "browserSentEvent" | "queryChange",
    ignoreIdOfPendingRemoval?: number | null,
  ) {
    tabs = await getNonPendingTabsOfCurrentWindow(
      searchQuery,
      ignoreIdOfPendingRemoval,
    );
  ...
  onMount(() => {
    browser.tabs.onCreated.addListener(() =>
      updateList(searchQuery, "browserSentEvent"),
    );
